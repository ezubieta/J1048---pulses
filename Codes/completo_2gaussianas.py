...
def gaussian_with_constant(x, a, mu, sigma, c,a2,mu2,sigma2):
	return a * np.exp(-(x - mu)**2 / (2 * sigma**2)) + c + a2 * np.exp(-(x - mu2)**2 / (2 * sigma2**2))
...
def ajustar(x, y, centro):
	...
	initial_guess = [a0, x0, s0, c0,a0,190,s0]
	boundmin = [0, 170, 0.1, c0 - c_, 0, 178, 0.1]
	boundmax = [1.1*a0, 182, 10, c0 + c_, 1.1*a0, 200, 10]
...
def generar_ajuste(nombre_archivo):
	...
	dof=len(signal_v2)-4 #number of params
	chisq_red=chi2/dof
	return params[0],params[2], params[3],params[4],params[6],errores[0],errores[2], errores[3], errores[4],errores[6],chisq_red,t_obs, params[1], params[5],errores[1],errores[5]
...
def generar_grafico(nombre_archivo):
	...
	y_gauss1 = gaussian_with_constant(x_ajuste, params[0], params[1],params[2], params[3],0,0,1)
	y_gauss2 = gaussian_with_constant(x_ajuste, 0,0,1, params[3], params[4],params[5], params[6])
...
for i in range(len(c)):
	...
	centrob=ufloat(centro2[i],err_centro2[i])
